...
const PROJECT_VIEW_STEPS = [
  {
    targetId: 'tour-project-export-actions',
    icon: '📅',
    tag: 'Step 1 of 4 · Export & Sync',
    title: 'Calendar & Team Sharing Hub',
    ...
    tip: 'Your exported calendar events include calculated start-by dates!',
    preferredPlacement: 'bottom',
  },
  {
    targetId: 'tour-project-members-section',
    icon: '👥',
    tag: 'Step 2 of 4 · Team Balance',
    title: 'Teammate Workload Balancing',
    ...
    tip: 'Tasks are automatically suggested to teammates with the most free hours.',
    preferredPlacement: 'bottom',
  },
  {
    targetId: 'tour-project-task-controls',
    icon: '🔍',
    tag: 'Step 3 of 4 · Task Search & Filters',
    title: 'Filter & Sort Assignments',
    desc: 'Quickly find tasks by name or assignee, filter by status, and sort by calculated start-by urgency.',
    tip: 'Use the search box to immediately find tasks in large projects.',
    preferredPlacement: 'bottom',
  },
  {
    targetId: 'tour-project-add-task-btn',
    icon: '⏱️',
    tag: 'Step 4 of 4 · Add Tasks & Buffers',
    title: 'Add Assignments & Auto-Buffer',
    ...
    tip: 'Press N on your keyboard anywhere on this page to quickly open the task drawer.',
    preferredPlacement: 'bottom',
  },
]
...
const priorityStyles = {
  low: 'bg-paper-dim text-graphite',
  medium: 'bg-highlight-soft text-ink',
  high: 'bg-deadline-soft text-deadline',
}
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <div className="sm:col-span-2">
        ...
        <input
          ...
          className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer"
          placeholder="e.g. Draft literature review"
        />
      </div>
      <div>
        ...
        <DatePicker
          value={form.deadline}
          onChange={(iso) => form.setDeadline(iso)}
          min={form.todayIso}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="flex items-center gap-2 cursor-pointer p-2 rounded-lg hover:bg-paper transition">
              <input
                ...
                name="assignee"
                value=""
                checked={form.assignedMemberId === ''}
                onChange={() => form.setAssignedMemberId('')}
                className="accent-buffer"
              />
            ...
            {suggested.map((m, i) => {
              ...
              return (
                <label
                  key={m.id}
                  className={`flex items-center gap-2 cursor-pointer p-2 rounded-lg hover:bg-paper transition ${
                    form.assignedMemberId === m.id ? 'bg-buffer-soft ring-1 ring-buffer/40' : ''
                  }`}
                >
                  <input
                    type="radio"
                    name="assignee"
                    value={m.id}
                    checked={form.assignedMemberId === m.id}
                    onChange={() => form.setAssignedMemberId(m.id)}
                    className="accent-buffer"
                  />
                  <span className="flex-1 min-w-0">
                    <span className="text-sm text-ink flex items-center gap-1.5">
                      {m.display_name}
                      {isBest && (
                        <span className="text-[10px] font-medium bg-buffer text-white px-1.5 py-0.5 rounded-full">
                          suggested
                        </span>
                      )}
                      {stats.overloaded && (
                        <span className="text-[10px] font-medium bg-deadline-soft text-deadline px-1.5 py-0.5 rounded-full">
                          overloaded
                        </span>
                      )}
                    </span>
      ...
      <div className="sm:col-span-2 flex gap-2 pt-2">
        <button
          type="submit"
          disabled={form.taskSubmitting}
          className="bg-ink text-paper rounded-lg px-4 py-2 text-sm font-medium hover:bg-ink-soft disabled:opacity-50 transition"
        >
...
export default function ProjectView() {
  ...
  useEffect(() => {
    const hasSeenTour = localStorage.getItem('deadline_buffer_spotlight_projectview_v1')
    if (!hasSeenTour) {
  ...
  // Open add drawer (keyboard shortcut: N)
  useEffect(() => {
  ...
  function requestDeleteTask(task) {
    setConfirmTarget({ kind: 'task', id: task.id, label: task.name })
  }
  ...
  function requestRemoveMember(member) {
    const assignedCount = tasks.filter((t) => t.assigned_member_id === member.id).length
    setConfirmTarget({ kind: 'member', id: member.id, label: member.display_name, assignedCount })
  }
  ...
  async function confirmRemoveMember() {
    ...
    setConfirmTarget(null)
    if (error) { toast.error(error.message); return }
    toast.success(`${confirmTarget.label} removed.`)
    reload()
  }
  ...
  if (loadError) {
    return (
      <div className="max-w-md mx-auto mt-10 text-center">
        <p className="text-sm text-deadline bg-deadline-soft border border-deadline/20 rounded-lg px-4 py-3 mb-3">
          {loadError}
        </p>
  ...
  const dueSoonCount = tasks.filter((t) => {
    ...
    return getDaysUntilDeadline(t.deadline) >= 0 && getDaysUntilDeadline(t.deadline) <= 7
  }).length
  ...
  let visibleTasks = tasks.filter((t) => {
    ...
    if (searchQuery.trim()) {
      ...
      if (!matchName && !assigneeName.includes(q)) return false
    }
    return true
  })
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      <header className="bg-white border-b border-ink/10 animate-fade-in">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            <Link to="/dashboard" className="text-sm text-graphite hover:text-ink transition-colors">
              ← Dashboard
            </Link>
            <Logo />
          </div>
          <div className="flex items-center justify-between gap-3 mt-3 flex-wrap">
            <div className="flex items-center gap-3 flex-wrap">
              {renaming ? (
                <form onSubmit={handleSaveRename} className="flex items-center gap-2">
                  <input
                    ...
                    className="font-display text-xl font-semibold text-ink border-b border-buffer focus:outline-none bg-transparent"
                  />
                  <button
                    type="submit"
                    disabled={renameSubmitting}
                    className="text-xs text-buffer hover:underline disabled:opacity-50"
                  >
                    Save
                  </button>
                  <button type="button" onClick={() => setRenaming(false)} className="text-xs text-graphite hover:underline">
                    Cancel
                  </button>
                </form>
              ) : (
                <>
                  ...
                  <button
                    ...
                    className="text-xs text-graphite/60 hover:text-buffer transition"
                  >
                    Rename
                  </button>
                </>
              ...
              <span
                className={`text-xs font-medium px-2 py-1 rounded-full ${
                  isGroup ? 'bg-highlight-soft text-ink' : 'bg-buffer-soft text-buffer'
                }`}
              >
                {isGroup ? 'Group' : 'Solo'}
              </span>
            ...
            <div id="tour-project-export-actions" className="flex items-center gap-2 flex-wrap">
              ...
              <button
                type="button"
                onClick={() => {
                  const summary = formatProjectSummary(project, tasks, members)
              ...
              <button
                type="button"
                onClick={() => {
                  exportProjectToIcs(project, tasks, members)
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        {totalTasks > 0 && (
          <section className="bg-white rounded-xl border border-ink/10 p-6 animate-fade-up delay-50">
            ...
            <div className="h-2 w-full bg-paper-dim rounded-full overflow-hidden">
              <div
                className="h-full bg-buffer animate-bar-grow rounded-full"
        ...
        {/* Members section — group projects only */}
        {isGroup && (
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-graphite shrink-0">
                        {stats.done}/{stats.assigned} tasks done
                      </span>
                      <button
            ...
            <form onSubmit={memberForm.handleAddMember} className="flex flex-wrap gap-2 items-end">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => memberForm.setMemberName(e.target.value)}
                  className="rounded-lg border border-ink/15 px-2 py-1.5 text-sm w-40 focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer"
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    max="80"
                    required
                    value={memberForm.memberHours}
                    onChange={(e) => memberForm.setMemberHours(e.target.value)}
                    className="rounded-lg border border-ink/15 pl-2.5 pr-12 py-1.5 text-sm w-28 focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer"
                  />
                  <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-graphite pointer-events-none font-medium">
                    hrs/wk
                  </span>
                </div>
              </div>
              <button
                type="submit"
                disabled={memberForm.memberSubmitting}
                className="bg-ink text-paper rounded-lg px-3 py-1.5 text-sm font-medium hover:bg-ink-soft disabled:opacity-50 transition"
              >
                {memberForm.memberSubmitting ? 'Adding…' : '+ Add member'}
              </button>
            </form>
            {memberForm.memberError && (
              <p className="text-xs text-deadline bg-deadline-soft border border-deadline/20 rounded-lg px-3 py-2 mt-2">
                {memberForm.memberError}
              </p>
            )}
          </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            ...
            <div id="tour-project-task-controls" className="flex flex-wrap gap-2 items-center">
                ...
                <>
                  <div className="relative min-w-[140px] flex-1 sm:flex-initial">
                    <input
                      ...
                      className="w-full text-xs rounded-lg border border-ink/15 pl-7 pr-2.5 py-1.5 bg-white text-ink placeholder:text-graphite/60 focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer transition"
                    />
                  ...
                  <select
                    ...
                    className="text-xs rounded-lg border border-ink/15 px-2 py-1.5 bg-white text-graphite"
                  >
                  ...
                  <select
                    ...
                    className="text-xs rounded-lg border border-ink/15 px-2 py-1.5 bg-white text-graphite"
                  >
                    <option value="start_by">Sort: start-by date</option>
              ...
              <button
                id="tour-project-add-task-btn"
                onClick={() => setDrawerMode('add')}
                className="bg-ink text-paper text-xs font-medium rounded-lg px-3 py-1.5 hover:bg-ink-soft transition"
              >
                + Add task
                <span className="ml-1.5 opacity-50 font-mono hidden sm:inline">N</span>
              </button>
            </div>
          </div>
          ...
          {tasks.length === 0 ? (
            <div className="bg-white rounded-2xl border border-ink/10 p-8 text-center shadow-xs animate-fade-up">
              ...
              <div className="flex flex-wrap justify-center items-center gap-2 mb-5">
                <button
                  type="button"
                  onClick={() => {
                    taskForm.setTaskName('Draft literature review')
                    taskForm.setHours('6')
                ...
                <button
                  type="button"
                  onClick={() => {
                    taskForm.setTaskName('Problem set solutions')
                    taskForm.setHours('4')
                    taskForm.setPriority('medium')
                    setDrawerMode('add')
                  }}
                  className="text-[11px] text-graphite hover:text-ink bg-paper hover:bg-paper-dim border border-ink/10 px-2.5 py-1 rounded-lg transition"
                >
            ...
            <div className="bg-white rounded-xl border border-ink/10 p-6 text-center">
              ...
              <button
                onClick={() => { setStatusFilter('all'); setMemberFilter('all'); setSearchQuery('') }}
                className="text-xs text-buffer hover:underline"
              >
                    ...
                    <li key={task.id} className="bg-white rounded-xl border border-buffer/40 p-4">
                      <form onSubmit={taskEdit.handleSaveEditTask} className="grid gap-3 sm:grid-cols-2">
                        <div className="sm:col-span-2">
                          ...
                          <input
                            required autoFocus
                            value={taskEdit.editFields.name}
                            onChange={(e) => taskEdit.setEditFields({ ...taskEdit.editFields, name: e.target.value })}
                            className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer"
                          />
                        </div>
                        <div>
                          <label className="block text-xs text-graphite mb-1">Deadline</label>
                          <DatePicker
                            value={taskEdit.editFields.deadline}
                            onChange={(iso) => taskEdit.setEditFields({ ...taskEdit.editFields, deadline: iso })}
                            min={todayIso}
                            placeholder="Pick a deadline"
                            required
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={taskEdit.editFields.estimated_hours}
                            onChange={(e) => taskEdit.setEditFields({ ...taskEdit.editFields, estimated_hours: e.target.value })}
                            className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer"
                          />
                        </div>
                        <div>
                          <label className="block text-xs text-graphite mb-1">Priority</label>
                          <select
                            value={taskEdit.editFields.priority}
                            onChange={(e) => taskEdit.setEditFields({ ...taskEdit.editFields, priority: e.target.value })}
                            className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer"
                          >
                            <option value="low">Low</option>
                            <option value="medium">Medium</option>
                        ...
                        {isGroup && (
                          <div>
                            ...
                            <select
                              value={taskEdit.editFields.assigned_member_id}
                              onChange={(e) => taskEdit.setEditFields({ ...taskEdit.editFields, assigned_member_id: e.target.value })}
                              className="w-full rounded-lg border border-ink/15 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-buffer/50 focus:border-buffer"
                            >
                        ...
                        <div className="sm:col-span-2 flex gap-2">
                          <button
                            type="submit"
                            disabled={taskEdit.editSubmitting}
                            className="bg-ink text-paper rounded-lg px-4 py-2 text-sm font-medium hover:bg-ink-soft disabled:opacity-50 transition"
                          >
                            {taskEdit.editSubmitting ? 'Saving…' : 'Save changes'}
                          </button>
                          <button
                            type="button"
                            onClick={taskEdit.handleCancelEditTask}
                            className="text-sm text-graphite hover:text-ink px-4 py-2 transition"
                          >
                            Cancel
                          </button>
                        </div>
                    ...
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div className="flex-1 min-w-[200px]">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span
                            className={`text-[10px] font-medium uppercase tracking-wide px-1.5 py-0.5 rounded ${priorityStyles[task.priority]}`}
                          >
                            {task.priority}
                          </span>
                        </div>
                        <p className="text-xs text-graphite mt-1">
                          ...
                          {isGroup && member && ` · ${member.display_name}`}
                          {isGroup && !member && ' · Unassigned'}
                        </p>

                        <div className="mt-3 max-w-sm">
                          <BufferBar
                            todayIso={todayIso}
                            startByDate={task.start_by_date}
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <select
                          ...
                          className="text-xs rounded-lg border border-ink/15 px-2 py-1 bg-white"
                        >
                          <option value="not_started">Not started</option>
                          <option value="in_progress">In progress</option>
                          <option value="done">Done</option>
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => taskEdit.handleStartEditTask(task)}
                            className="text-xs text-buffer hover:underline"
                          >
                            Edit
                          </button>
                          <button
                            ...
                            className="text-xs text-deadline hover:underline"
                          >
                            Delete
                          </button>
                        </div>
      ...
      <TaskDrawer
        open={drawerMode === 'add'}
        onClose={() => { setDrawerMode(null); taskForm.resetForm() }}
        title="Add a task"
      >
      ...
      <ConfirmDialog
        ...
        message={
          confirmTarget?.kind === 'task'
            ? `Delete "${confirmTarget?.label}"? This can't be undone.`
            : confirmTarget?.assignedCount > 0
              ? `${confirmTarget?.label} has ${confirmTarget.assignedCount} task(s) assigned. Removing them will unassign those tasks. Continue?`
              : `Remove ${confirmTarget?.label} from this project?`
        }
      ...
      {/* Canva-Style Spotlight Tour for Project View Page */}
      <ProductTour
        steps={PROJECT_VIEW_STEPS}
        tourKey="deadline_buffer_spotlight_projectview_v1"
        isOpen={tourOpen}
